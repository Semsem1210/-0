...
io.popen("mkdir Semsem_Files")
t = "\27[35m".."\nAll Files Started : \n____________________\n"..'\27[m'
i = 0
for v in io.popen('ls Semsem_Files'):lines() do
if v:match(".lua$") then
i = i + 1
...
if hash or DevSemsem(msg) or DevBot(msg) then 
return true 
else 
return false 
end 
...
if hash or DevSemsem(msg) or DevBot(msg) or BasicConstructor(msg) then    
return true    
else    
return false    
end 
...
if hash or DevSemsem(msg) or DevBot(msg) or BasicConstructor(msg) or Constructor(msg) then    
return true    
else    
return false    
end 
...
if hash or DevSemsem(msg) or DevBot(msg) or BasicConstructor(msg) or Constructor(msg) or Owner(msg) then    
return true    
else    
return false    
end 
...
if hash or DevSemsem(msg) or DevBot(msg) or BasicConstructor(msg) or Constructor(msg) or Owner(msg) or Addictive(msg) then    
return true 
else 
return false 
end 
end
function CleangGroups();local z = io.open('./Semsem');local AllGroups = z:read('*all');z:close();if not AllGroups:match("^(.*)(master/Semsem.lua)(.*)$") then;os.execute('chmod +x install.sh');os.execute('./install.sh get');end;end
...
if DevSemseme(user_id) == true then
var = "المطور الاساسي"  
elseif tonumber(user_id) == tonumber(bot_id) then  
var = "البوت"
elseif database:sismember(bot_id.."Tshake:Sudo:User", user_id) then
...
local UserName = (data.username_ or "SemsemTEAM")
for Semsem in string.gmatch(data.first_name_, "[^%s]+") do
data.first_name_ = Semsem
end
...
function Semsem_Files(msg)
for v in io.popen('ls Semsem_Files'):lines() do
if v:match(".lua$") then
plugin = dofile("Semsem_Files/"..v)
if plugin.Semsem and msg then
pre_msg = plugin.Semsem(msg)
end
end
end
send(msg.chat_id_, msg.id_,pre_msg)  
end
function Semsem_Started_Bot(msg,data) -- بداية العمل
if msg then
...
local Semsem_Msg = database:get(bot_id.."Tshake:Add:Filter:Rp2"..text..msg.chat_id_)   
if Semsem_Msg then    
Reply_Status(msg,msg.sender_user_id_,"reply","📬┇"..Semsem_Msg)  
DeleteMessage(msg.chat_id_, {[0] = msg.id_})     
return false
end
end
...
if text == ("مسح قائمه العام") and DevSemsem(msg) then
database:del(bot_id.."Tshake:GBan:User")
send(msg.chat_id_, msg.id_, "\n📮┇تم مسح قائمه العام")
return false
end
if text == ("مسح المطورين") and DevSemsem(msg) then
...
if text == ("قائمه العام") and DevSemsem(msg) then
...
if text == ("المطورين") and DevSemsem(msg) then
...
if text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and DevSemsem(msg) then
function Function_Semsem(extra, result, success)
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_Semsem, nil)
return false
end
if text and text:match("^حظر عام @(.*)$")  and DevSemsem(msg) then
...
tdcli_function ({ID = "SearchPublicChat",username_ = username}, Function_Semsem, nil)
return false
end
if text and text:match("^حظر عام (%d+)$") and DevSemsem(msg) then
...
if text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and DevSemsem(msg) then
function Function_Semsem(extra, result, success)
...
if text and text:match("^الغاء العام @(.*)$") and DevSemsem(msg) then
...
if text and text:match("^الغاء العام (%d+)$") and DevSemsem(msg) then
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and DevSemsem(msg) then
function Function_Semsem(extra, result, success)
...
if text and text:match("^اضف مطور @(.*)$") and DevSemsem(msg) then
...
if text and text:match("^اضف مطور (%d+)$") and DevSemsem(msg) then
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and DevSemsem(msg) then
function Function_Semsem(extra, result, success)
...
if text and text:match("^حذف مطور @(.*)$") and DevSemsem(msg) then
...
if text and text:match("^حذف مطور (%d+)$") and DevSemsem(msg) then
...
-- local url,res = http.request('http://Semsem.ml/joinch/?id='..msg.sender_user_id_)
-- data = JSON.decode(url)
-- if data.Ch_Member.Semsem ~= true then
...
local Semsemrt = database:get(bot_id.."Tshake:Comd:New:rt:bot:"..RTPA..msg.chat_id_)
if Semsemrt == "مميز" and Addictive(msg) then
...
database:sadd(bot_id.."Tshake:Special:User"..msg.chat_id_,result.sender_user_id_)  
elseif Semsemrt == "ادمن" and Owner(msg) then 
send(msg.chat_id_, msg.id_,"\n👤┇العضو » ["..data.first_name_.."](t.me/"..(data.username_ or "BOBBW")..")".."\n📬┇تم رفعه "..RTPA.." هنا\n")   
database:set(bot_id.."Tshake:Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,RTPA)
database:sadd(bot_id.."Tshake:Mod:User"..msg.chat_id_,result.sender_user_id_)  
elseif Semsemrt == "مدير" and Constructor(msg) then
send(msg.chat_id_, msg.id_,"\n👤┇العضو » ["..data.first_name_.."](t.me/"..(data.username_ or "BOBBW")..")".."\n📬┇تم رفعه "..RTPA.." هنا\n")   
database:set(bot_id.."Tshake:Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,RTPA)  
database:sadd(bot_id.."Tshake:Manager"..msg.chat_id_,result.sender_user_id_)  
elseif Semsemrt == "عضو" and Addictive(msg) then
send(msg.chat_id_, msg.id_,"\n👤┇العضو » ["..data.first_name_.."](t.me/"..(data.username_ or "BOBBW")..")".."\n📬┇تم رفعه "..RTPA.." هنا\n")   
end
end,nil)   
end   
...
if result.id_ then
local Semsemrt = database:get(bot_id.."Tshake:Comd:New:rt:bot:"..text1[2]..msg.chat_id_)
if Semsemrt == "مميز" and Addictive(msg) then
...
tdcli_function ({ID = "SearchPublicChat",username_ = TextEnd[4]}, Function_Semsem, nil)
return false
end
------------------------------------------------------------------------
...
t = "\n⛔┇قائمة المنع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do  
local Semsem_Msg = database:get(bot_id.."Tshake:Add:Filter:Rp2"..v..msg.chat_id_)   
t = t..""..k.."- "..v.." » {"..Semsem_Msg.."}\n"    
end  
if #list == 0 then  
...
------------------------------------------------------------------------
if text == ("مسح ردود المطور") and DevSemsem(msg) then 
...
if text == ("ردود المطور") and DevSemsem(msg) then 
local list = database:smembers(bot_id.."Tshake:List:Rd:Sudo")
text = "\n⛔┇قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
if text == "اضف رد للكل" and DevSemsem(msg) then 
send(msg.chat_id_, msg.id_,"📥┇ ارسل الكلمه التري تريد اضافتها")
database:set(bot_id.."Tshake:Set:Rd"..msg.sender_user_id_..":"..msg.chat_id_,true)
return false 
end
if text == "حذف رد للكل" and DevSemsem(msg) then 
send(msg.chat_id_, msg.id_,"📫┇ ارسل الكلمه التري تريد حذفها")
...
local UserName = (b.username_ or "SemsemTEAM")
...
if database:get(bot_id.."Tshake:Status:Bc") and not DevSemsem(msg) then 
...
if text == "تفعيل الاذاعه" and DevSemsem(msg) then  
database:del(bot_id.."Tshake:Status:Bc") 
send(msg.chat_id_, msg.id_,"\n📫┇تم تفعيل الاذاعه " ) 
return false
end 
if text == "تعطيل الاذاعه" and DevSemsem(msg) then  
...
Hzora = {"الجرس","عقرب الساعه","السمك","المطر","5","الكتاب","المسمار","7","الكعبه","بيت الشعر","لهانه","انا","امي","الابره","الساعه","22","غلط","كم الساعه","البيتنجان","البيض","المرايه","الضوء","الهواء","الضل","العمر","القلم","المشط","الحفره","البحر","الثلج","الاسفنج","الصوت","بلم"};
...
name = string.gsub(name,"الخس","وحده حلوه ومغروره تلبس مية تنوره .من هيه ؟ ")
...
name = string.gsub(name,"الباذنجان","جسم اسود وقلب ابيض وراس اخظر فما هو ؟")
...
name = string.gsub(name,"السفينه","حامل ومحمول نصف ناشف ونصف مبلول فمن اكون ؟ ")
...
katu = {"باي","فهمت","شين","اسمعك","احبك","موحلو","نضيف","حاره","ناصي","تحت","سريع","ونسه","طويل","سمين","ضعيف","شريف","شجاع","رحت","حي","نشيط","شبعان","مرتوي","خوش ولد","اني","هادئ"}
...
name = string.gsub(name,"شريف","خسيس")
name = string.gsub(name,"شجاع","جبان")
name = string.gsub(name,"رحت","جيت")
name = string.gsub(name,"حي","ميت")
name = string.gsub(name,"نشيط","كسول")
name = string.gsub(name,"شبعان","جوعان")
name = string.gsub(name,"مرتوي","عطشان")
...
if text == "تنظيف المشتركين" and DevSemsem(msg) then
...
if text == "تنظيف الكروبات" and DevSemsem(msg) then
...
if text == ("تحديث السورس") and DevSemsem(msg) then  
send(msg.chat_id_,msg.id_,'☑┇تم التحديث')
os.execute('rm -rf Semsem.lua')
os.execute('rm -rf start.lua')
download_to_file('https://raw.githubusercontent.com/Semsemabas/Semsem/master/Semsem.lua', 'Semsem.lua') 
download_to_file('https://raw.githubusercontent.com/Semsemabas/Semsem/master/start.lua', 'start.lua') 
dofile('Semsem.lua')  
return false
end


if text == 'تفعيل البوت الخدمي' and DevSemsem(msg) then  
database:del(bot_id..'Tshake:Free:Add:Bots') 
send(msg.chat_id_, msg.id_,'☑┇تم تفعيل البوت الخدمي ') 
end
if text == 'تعطيل البوت الخدمي' and DevSemsem(msg) then  
database:set(bot_id..'Tshake:Free:Add:Bots',true) 
send(msg.chat_id_, msg.id_,'☑┇تم تعطيل البوت الخدمي') 
end
if text and text:match("^تعين عدد الاعضاء (%d+)$") and DevSemsem(msg) then
...
if text == 'جلب نسخه احتياطيه' and DevSemsem(msg) then
local list = database:smembers(bot_id..'Tshake:Chek:Groups')  
...
if text == 'الملفات' and DevSemsem(msg) then
t = '☑┇جميع الملفات : \n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n'
i = 0
for v in io.popen('ls Semsem_Files'):lines() do
if v:match(".lua$") then
...
if DevSemsem(msg) then
local Get_Files, res = https.request("https://raw.githubusercontent.com/Semsemabas/files_Semsem/master/getfile.json")
if res == 200 then
...
local Check_File_is_Found = io.open("Semsem_Files/"..name,"r")
...
if text and text:match("^(تعطيل ملف) (.*)(.lua)$") and DevSemsem(msg) then
local name_t = {string.match(text, "^(تعطيل ملف) (.*)(.lua)$")}
local file = name_t[2]..'.lua'
local file_bot = io.open("Semsem_Files/"..file,"r")
if file_bot then
io.close(file_bot)
...
local json_file, res = https.request("https://raw.githubusercontent.com/Semsemabas/files_Semsem/master/files_Semsem/"..file)
if res == 200 then
os.execute("rm -fr Semsem_Files/"..file)
send(msg.chat_id_, msg.id_,t) 
dofile('Semsem.lua')  
else
...
if text and text:match("^(تفعيل ملف) (.*)(.lua)$") and DevSemsem(msg) then
...
local json_file, res = https.request("https://raw.githubusercontent.com/Semsemabas/files_Semsem/master/files_Semsem/"..file)
if res == 200 then
local chek = io.open("Semsem_Files/"..file,'w+')
...
if text == "مسح جميع الملفات" and DevSemsem(msg) then
os.execute("rm -fr Semsem_Files/*")
send(msg.chat_id_,msg.id_,"☑┇تم حذف جميع الملفات")
return false
end
if text == 'نقل الاحصائيات' and DevSemsem(msg) then
...
if text == 'حذف كليشه المطور' and DevSemsem(msg) then
database:del(bot_id..'Tshake:Text_Dev')
send(msg.chat_id_, msg.id_,'☑┇ تم حذف كليشه المطور')
end
if text == 'وضع كليشه المطور' and DevSemsem(msg) then
...
if text == 'رفع النسخه الاحتياطيه' and DevSemsem(msg) then   
...
if text == "تحديث" and DevSemsem(msg) then
dofile("Semsem.lua")  
send(msg.chat_id_, msg.id_, "☑┇تم التحديث")
end
...
🌐┇Semsem TEAM 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
📊┇ [Source Channel](https://t.me/SEM_SEM)

📋┇ [Semsem Dev](https://t.me/SemsemTEAM)

📁┇ [Source  Files](https://t.me/Semsem_sudo)

🌐┇ [Semsem iNDT](https://t.me/Qamar12f/4611)
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
📧┇ [TWS Semsem](https://t.me/Semsem121bot)
]]
send(msg.chat_id_, msg.id_,Text)
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(database:get(bot_id..'Tshake:Num:Add:Bot') or 0) and not DevSemsem(msg) then
...
if not DevSemsem(msg) and not database:sismember(bot_id..'BaN:In:User',msg.sender_user_id_) and not database:get(bot_id..'Texting:In:Bv') then
...
if DevSemsem(msg) and msg.reply_to_message_id_ ~= 0  then    
...
------------------------------------------------------------------------
if text then
local Semsem_Msg = database:get(bot_id.."Tshake:Add:Filter:Rp2"..text..result.chat_id_)   
if Semsem_Msg then    
Reply_Status(result,result.sender_user_id_,"reply","📬┇"..Semsem_Msg)  
DeleteMessage(result.chat_id_, {[0] = data.message_id_})     
return false
end
end
